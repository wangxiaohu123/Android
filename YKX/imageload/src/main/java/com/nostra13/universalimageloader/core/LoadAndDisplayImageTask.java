...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	public LoadAndDisplayImageTask(String diskKey,ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo, Handler handler) {
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		...
		try {
			...
			if ((diskKey!=null)&&(diskKey.length()>0)){
				imageFile = configuration.diskCache.get(diskKey);
			}
	...
	private boolean downloadImage() throws IOException {
		...
		if (is == null) {
			...
			try {
				if ((diskKey!=null)&&(diskKey.length()>0)){
					return configuration.diskCache.save(diskKey, is, this);
				}
	...
	private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
		...
		if ((diskKey!=null)&&(diskKey.length()>0)){
			targetFile = configuration.diskCache.get(diskKey);
		}
		if (targetFile != null && targetFile.exists()) {
			...
			if (bmp != null) {
				if ((diskKey!=null)&&(diskKey.length()>0)){
					saved = configuration.diskCache.save(diskKey, bmp);
				}else{
