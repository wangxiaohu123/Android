...
public class ImageLoader {
	...
	public void displayImage(String key,String uri, ImageAware imageAware, DisplayImageOptions options,
							 ImageLoadingListener listener, ImageLoadingProgressListener progressListener) {
		displayImage(key,uri, imageAware, options, null, listener, progressListener);
	}
	...
	public void displayImage(String key,String uri, ImageAware imageAware, DisplayImageOptions options,
		...
		if (bmp != null && !bmp.isRecycled()) {
			...
			ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
					options, listener, progressListener, engine.getLockForUri(uri));
			LoadAndDisplayImageTask displayTask = new LoadAndDisplayImageTask(key,engine, imageLoadingInfo,
					defineHandler(options));
			if (options.isSyncLoading()) {
				displayTask.run();
			} else {
	...
	public void displayImage(String key,String uri, ImageView imageView, DisplayImageOptions options) {
		displayImage(key,uri, new ImageViewAware(imageView), options, null, null);
	}
