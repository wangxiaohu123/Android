...
public abstract class PlatformPage extends OnekeySharePage {
	...
	public void onCreate() {
		...
		addHeaderView(llPanel);
		addNullView(llPanel);
		llPage.addView(llPanel, lp);

		MobViewPager mvp = new MobViewPager(activity);
//		mvp.setPadding(0,dip2px(activity,60),0,dip2px(activity,60));
		...
		addNullView(llPanel);
		addButtomView(llPanel);
	}
	/**
	 * 根据手机的分辨率从 dp 的单位 转成为 px(像素)
	 */
	public int dip2px(Context context, final float dpValue) {
		float density = context.getResources().getDisplayMetrics().density;
		return (int) (dpValue * density + 0.5f);
	}


	public void addNullView(LinearLayout llPage){

		LinearLayout.LayoutParams params=new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT, dip2px(activity,30));
		params.setMargins(0,-dip2px(activity,2),0,0);
	...
	public void addHeaderView(LinearLayout llPage){

		LinearLayout.LayoutParams params=new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT, dip2px(activity,50));
	...
	public void addButtomView(LinearLayout llPage){
		...
		line.setBackgroundColor(activity.getResources().getColor(R.color.view_line_color));
		LinearLayout.LayoutParams paramsline=new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT, 1);
		llPage.addView(line,paramsline);
