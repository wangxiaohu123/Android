...
public class WheelView extends View {
    ...
    private static final float SCALECONTENT = 0.95F;//非中间文字则用此控制高度，压扁形成3d错觉
    ...
    private void remeasure() {//重新测量
        ...
        int dury = dip2px(context,6);
        //计算两条横线 和 选中项画笔的基线Y位置
        firstLineY = (measuredHeight - itemHeight) / 2.0F+dury;
        secondLineY = (measuredHeight + itemHeight) / 2.0F+dury;
    ...
    private void measureTextWidthHeight() {
        ...
        for (int i = 0; i < adapter.getItemsCount(); i++) {
          ...
          /* int textHeight = rect.height();*/
            ...
            maxTextHeight = rect.height() + 2+dip2px(context,15);
        ...
        itemHeight = lineSpacingMultiplier * maxTextHeight+dip2px(context,3);
    ...
    private int durWidth=0;

    public void resetDurWidth(int durWidth){

        this.durWidth=durWidth;
    }

    private boolean isDrawFlag=true;

    public void setDrawFlag(boolean isDrawFlag){
        this.isDrawFlag=isDrawFlag;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Paint rectpaintx = new Paint();
        rectpaintx.setColor(getResources().getColor(R.color.theme_main_line_color));// 设置灰色
        rectpaintx.setStyle(Paint.Style.FILL);//设置填满
        rectpaintx.setAntiAlias(true);
        canvas.drawRect(new RectF(0,0,measuredWidth,measuredHeight),rectpaintx);
        ...
        canvas.drawRect(new RectF(0,firstLineY,measuredWidth,secondLineY),rectpaint);

        Paint linepaint = new Paint();
        linepaint.setColor(getResources().getColor(R.color.theme_main_color));
        linepaint.setAntiAlias(true);
        linepaint.setTextScaleX(paintIndicator.getTextScaleX());
        linepaint.setTypeface(paintIndicator.getTypeface());
        linepaint.setTextSize(paintIndicator.getTextSize());


        if (isDrawFlag) {
            canvas.drawLine(measuredWidth - dip2px(context, 1), 0, measuredWidth, measuredHeight, linepaint);
        }
        ...
        if (dividerType == DividerType.WRAP){
            ...
            canvas.drawLine(startX, firstLineY, endX, firstLineY, linepaint);
            canvas.drawLine(startX, secondLineY, endX, secondLineY, linepaint);
        }else {
        ...
        if (!TextUtils.isEmpty(label)) {
            ...
            tempaint.setColor(getResources().getColor(R.color.theme_main_background_color));
            tempaint.setAntiAlias(true);
            tempaint.setTextScaleX(1.1F);
            tempaint.setTypeface(typeface);
            tempaint.setTextSize(dip2px(context,7));
            int dur=measuredWidth-dip2px(context,120);
            if (dur<=100){
                dur=0;
            }
            canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET-dip2px(context,41)-dur-this.durWidth, centerY-dip2px(context,13), tempaint);
        }
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    linepaint.setTextSize(dip2px(context,18));
                    linepaint.setColor(getResources().getColor(R.color.theme_main_background_color));
                    canvas.drawText(contentText, drawCenterContentStart, Y, linepaint);
...
//                    canvas.scale(1.0F, -(float) Math.sin(radian*0.5) * SCALECONTENT);
                    paintOuterText.setTextSize(dip2px(context,14));
                    canvas.drawText(contentText, drawOutContentStart, maxTextHeight, paintOuterText);
//                    canvas.restore();
                }
                canvas.restore();
